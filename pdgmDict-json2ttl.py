...
"""
    lprefix = str('@prefix ' + jdata["lgpref"] + ':     <http://id.oi.uchicago.edu/aama/2013/' + lang + '/> .\n\n')
    ...
    linfo = linfo + str(aamaLang + ' aamas:lpref "' +  jdata["lgpref"] + '" .\n')
    linfo = linfo + str(aamaLang + ' aamas:dataSource "' +  jdata["datasource"] + '" .\n')
    linfo = linfo + str(aamaLang + ' aamas:dataSourceNotes "' +  jdata["datasourceNotes"] + '" .\n')
    linfo = linfo + str(aamaLang + ' aamas:geodemoURL "' +  jdata["geodemoURL"] + '" .\n')
    linfo = linfo + str(aamaLang + ' aamas:geodemoTxt "' +  jdata["geodemoTXT"] + '" .\n')
    trans = "\n#TRANSCRIPTION INFO:\n"
    try:
        ### Transcription for the moment is not transformed into proper ttl
        ### but is inserted into the ttl file as an unanalyzed string. The 
        ### proper ttl format
        ### for the transcription table has yet to be determined.
        transcription = jdata["transcription"]
        for prop in  transcription.keys():
            val = transcription.get(prop)
            trans  = trans + str( aamaLang + ' aamas:' + prop + ' "' + str(val) + '" .\n')
    except:
        trans = trans + str("#[Not yet supplied]\n")
    prelude = header + prefixes + lprefix + linfo + trans
    return(prelude)
...
def doLexemes(lang, jdata):
    ...
    for lex in lexentries.keys():
        ...
        # 6/19/23 Changed from 'aama:LANG-lex a aamas:Lexeme'
        sec = str(lpref +':lex-' + lex + ' a aamas:Lexeme ;\n')
    ...
    for tcl in range(len(tclusters)):
        ...
        for term in data:
            ...
            for cprop in common:
                ...
                if cprop == 'lexeme':
                    sec += str('\taamas:' + cprop + ' ' + lpref + ':lex-' + cval + ' ;\n')
                elif cprop[0:5] == 'token' or cprop == 'stemAlt':
            ...
            for prop in vprops:
                ...
                if prop[0:5] == 'token':
                    ...
                    sec += str('\taamas:' + prop + ' ' + lpref + ':lex-' + pdict[prop] + ' ;\n')
                else:
