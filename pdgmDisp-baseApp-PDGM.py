...
def displayLInfo(*args):
    # For the display of family and geodemo information.
    # Set up for queries
...
def displaySInfo(*args):
    # For the display of bibliographic source information.
    sparql = SPARQLWrapper("http://localhost:3030/aama/query")
    l = str(lmsg.get())
    Lang = l.capitalize()
...
def displayPDGM(*args):   
    ...
    # 1.) Get SPARQL query for source + lexeme & gloss if present
    if "lexeme" in pvalue:
...
def pdgmCombine(*args):
    ...
    #pdgmstr3 = StringIO(pdgmstr2)
    #print("pdgmstr3:")
    #print(pdgmstr3)
    #Argument of 'read_csv' has to be a file
    pdgmpd = pd.read_csv(StringIO(pdgmstr2))
    #pdgmpd = pd.read_csv(pdgmstr2)
    ...
    print(str("vallist: " + str(vallist)))
    print(str("pivlist: " + str(pivlist)))
    ...
    print(ppiv1)
    print("\nCombined: paradigm dataform pivoted on 'token' (w/o index):")
    print(ppiv2)

    # 09/13/25: For the moment will not try to reformat pandas pivot
    # table. Leave 'native' pandas formatting and decide later ho2
    # final tables should look.
    ## Now do left-align
    ## by formatting
    #print("\nMethod4: formatting")
    #formatted_pdgmpd = ppiv2.map(lambda x: f"{x:<20}")
    ...
    formattedStr = str(ppiv2)
    print(formattedStr)
    p3string = formattedStr.replace('NaN', '___')
    print("missing data: replace 'nan'(<'NaN') by '___'")
    print(p3string)
...
pdbutton = ttk.Button(cframe, text="Display Paradigm Info", command=displayPInfo)
